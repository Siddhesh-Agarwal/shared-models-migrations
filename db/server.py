...
class ServerQueries:
        ...
        try:
            async with self.session() as session:
                ...
                for metric in discord_metrics:
                    stmt = select(DiscordMetrics).where(
                        DiscordMetrics.product_name == metric["product_name"]
                    )
                    result = await session.execute(stmt)
        ...
        try:
            async with self.session() as session:
                for metric in github_metrics:
                    ...
                    stmt = select(GithubMetrics).where(
                        GithubMetrics.product_name == metric["product_name"]
                    )
                    result = await session.execute(stmt)
                    existing_record = result.scalars().first()
        ...
        try:
            ...
            async with self.session() as session:
                stmt = select(
                    exists()
                    .where(
                        (GithubClassroomData.discord_id.is_(None))
                        | (GithubClassroomData.discord_id == discord_id)
                    )
    ...
    async def save_classroom_records(self, data):
        try:
            async with self.session() as session:
                for record in data:
                    try:
                        new_record = GithubClassroomData(**record)
                        session.add(new_record)
            ...
            for record in data:
                try:
                    stmt = (
                        ...
                        .where(
                            GithubClassroomData.assignment_id
                            == record.get("assignment_id"),
                            GithubClassroomData.discord_id
                            == cast(str(record.get("discord_id")), String),
                        )
                        .values(
                            assignment_name=record.get("assignment", {}).get("title"),
                            assignment_url=record.get("assignment", {})
                            .get("classroom", {})
                            .get("url"),
                            c4gt_points=record.get("c4gt_points"),
                            github_username=record.get("students", [{}])[0].get(
                                "login"
                            ),
                            ...
                            starter_code_url=record.get(
                                "starter_code_url",
                                record.get("repository", {}).get("html_url"),
                            ),
                            student_repository_name=record.get("repository", {}).get(
                                "full_name"
                            ),
                            student_repository_url=record.get("repository", {}).get(
                                "html_url"
                            ),
                            submission_timestamp=record.get(
                                "submission_timestamsp", datetime.now()
                            ),
    ...
    async def insert_org(self, name):
        try:
            async with self.session() as session:
                ...
                if not table:
                    raise ValueError("No ORM class found for table community_orgs")

                stmt = insert(table).values(name=name).returning(table)
                ...
                print(
                    "inserted_record ",
                    {"id": inserted_record[0], "name": inserted_record[1]},
                )
            ...
            async with self.session() as session:
                ...
                for table_name in tables_to_check:
                    ...
                    if not table_class:
                        continue
                    stmt = select(table_class).where(
                        getattr(table_class, "issue_id") == issue_id
                    )
                    result = await session.execute(stmt)
        ...
        try:
            async with self.session() as session:
                ...
                stmt = (
                    update(table)
                    .where(
                        table.issue_id == data["issue_id"]
                    )  # Match the existing issue by issue_id
                    .values(
                        link=data["link"],
                        labels=cast(
                            data["labels"], ARRAY(String)
    ...
    async def update_pr_data(self, data, table_name):
        try:
            ...
            async with self.session() as session:
                ...
                stmt = (
                    update(table_class)
                    .where(table_class.pr_id == new_pr_history.pr_id)
                    .values()
        ...
        try:
            async with self.session() as session:
                ...
                if pr_history_record:
                    ...
                    pr_history_record.merged_at = (
                        None if data["merged_at"] is None else data["merged_at"]
                    )
    ...
    async def addPr(self, prData, issue_id):
        try:
            if issue_id:
                ...
                if len(ticket) == 0:
                    ticket = await self.get_data(
                        "issue_id", "dmp_tickets", issue_id, None
                    )
            ...
            for pr in prData:
                ...
                await self.add_data(data, "connected_prs")
        ...
        try:
            async with self.session() as session:
                ...
                stmt = select(table).where(
                    and_(table.complexity == complexity_type, table.role == type)
                )
                result = await session.execute(stmt)
    ...
    async def upsert_point_transaction(
        self, issue_id, user_id, points, user_type="Contributor"
    ):
        try:
            async with self.session() as session:
                ...
                stmt = select(table).where(
                    and_(table.issue_id == issue_id, chosen_column == user_id)
                )
                ...
                if transaction:
                    ...
                    update_stmt = (
                        update(table)
                        .where(
                            and_(table.issue_id == issue_id, table.user_id == user_id)
                        )
                        .values(point=points)
                    )
    ...
    async def deleteIssueComment(self, commentId):
        try:
            async with self.session() as session:
                ...
                # Build and execute the query with multiple conditions
                stmt = delete(table).where(getattr(table, "id") == commentId)
                result = await session.execute(stmt)
        ...
        try:
            async with self.session() as session:
                ...
                stmt = (
                    ...
                    .join(
                        orgs_alias, Issues.org_id == orgs_alias.id, isouter=True
                    )  # Left join with CommunityOrgs
                    .join(
                        points_alias,
                        Issues.complexity == points_alias.complexity,
                        isouter=True,
                    )  # Left join with PointSystem
                )
    ...
    async def get_joined_data_with_filters(self, filters=None):
        async with self.session() as session:
            ...
            query = (
                select(issues, orgs, points)
                .join(orgs, issues.org_id == orgs.id)
                .join(points, points.complexity == issues.complexity)
            )
            ...
            return [
                dict(
                    issue=record[0].to_dict(),
                    org=record[1].to_dict(),
                    points=record[2].to_dict(),
                )
        ...
        try:
            async with self.session() as session:
                ...
                query = (
                    ...
                    .outerjoin(
                        IssueContributors, Issues.id == IssueContributors.issue_id
                    )
                    .outerjoin(
                        ContributorsRegistration,
                ...
                for row in rows:
                    ...
                    contributors_registration = (
                        row.ContributorsRegistration.to_dict()
                        if row.ContributorsRegistration
                        else None
                    )
                    data.append(
                        {
    ...
    def add_github_user(self, user):
        data = (
            ...
            .upsert(user, on_conflict=["github_id", "discord_id"])
            .execute()
        )
        return data.data
