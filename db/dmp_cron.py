...
class DmpCronQueries:
    async def get_timestamp(async_session, model, col_name: str, col: str, value):
        try:
            ...
            query = select(getattr(model, col_name)).filter(
        ...
        try:
            async with async_session() as session:
                ...
                query = (
                    select(
                        ...
                        func.json_build_object(
                            "created_at",
                            dmp_org_alias.created_at,
                            "description",
                            dmp_org_alias.description,
                            "id",
                            dmp_org_alias.id,
                            "link",
                            dmp_org_alias.link,
                ...
                for row in rows:
                    ...
                    dmp_orgs = issue_dict.pop(
                        "dmp_orgs"
                    )  # Extract JSON object from row
    ...
    async def upsert_pr_update(async_session, pr_update_data):
        try:
            async with async_session() as session:
                async with session.begin():
                    pr_update_data["pr_updated_at"] = (
                        datetime.fromisoformat(pr_update_data["pr_updated_at"]).replace(
                            tzinfo=None
                        )
                        if pr_update_data["pr_updated_at"]
                        else None
                    )
                    pr_update_data["merged_at"] = (
                        datetime.fromisoformat(pr_update_data["merged_at"]).replace(
                            tzinfo=None
                        )
                        if pr_update_data["merged_at"]
                        else None
                    )
                    pr_update_data["closed_at"] = (
                        datetime.fromisoformat(pr_update_data["closed_at"]).replace(
                            tzinfo=None
                        )
                        if pr_update_data["closed_at"]
                        else None
                    )
    ...
    async def get_week_updates(async_session, dmp_id, week):
        try:
            async with async_session() as session:
                # Build the ORM query
                stmt = select(DmpWeekUpdates).where(
                    DmpWeekUpdates.dmp_id == dmp_id, DmpWeekUpdates.week == week
                )
                # Execute the query
                result = await session.execute(stmt)
